...
import (
	...
	"github.com/juanescendales/go-concurrency-http-practice/db"
	"github.com/juanescendales/go-concurrency-http-practice/models"
	"github.com/juanescendales/go-concurrency-http-practice/stats"
)
...
func New() (Repo, error) {
	...
	o := repo{
		...
		reversing: make(chan models.Order),
		done:      done,
		processed: processed,
	}
...
func (r repo) RequestReversal(orderId string) (*models.Order, error) {
	...
	err = order.RequestReverse()
	if err != nil {
		return &models.Order{}, err
	}
	select {
	case r.reversing <- order:
		r.orders.Upsert(order)
		return &order, nil
	case <-r.done:
		return nil, fmt.Errorf("orders app is closed, try again later")
...
func (r repo) processReversals() {
	fmt.Println("Reversal processing service started!")
	for {
		select {
		case order := <-r.reversing:
			r.processReversal(&order)
			r.orders.Upsert(order)
			fmt.Printf("Reversing order %s completed\n", order.ID)
			r.processed <- order
		case <-r.done:
			fmt.Println("Order processing stopped!")
			return
...
func (r repo) processReversal(order *models.Order) {
	item := order.Item
	product, err := r.products.Find(item.ProductID)
	if err != nil {
		order.Status = models.OrderStatus_Completed
		order.Error = err.Error()
		return
	}
